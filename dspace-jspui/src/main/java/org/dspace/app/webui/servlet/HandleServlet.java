...
import org.dspace.app.webui.factory.SearchRequestProcessorWithManagerFactory;
 ...
 * extracted from the URL, e.g: <code>/community/1721.1/1234</code>. If there is
 * anything after the Handle, the request is forwarded to the appropriate
 * servlet. For example:
 * <P>
...
public class HandleServlet extends DSpaceServlet {
	...
	protected void doDSGet(Context context, HttpServletRequest request,
		...
		if (dso == null) {
			...
			JSPManager.showInvalidIDError(request, response,
					StringEscapeUtils.escapeHtml(path), -1);
		...
		if ("/statistics".equals(extraPathInfo)) {
			...
			RequestDispatcher dispatch = getServletContext()
					.getNamedDispatcher("displaystats");
		...
		if (dso.getType() == Constants.ITEM) {
			// do we actually want to display the item, or forward to another
			// page?
	...
	private void displayItem(Context context, HttpServletRequest request,
		...
		if (!ConfigurationManager.getBooleanProperty("webui.suggest.enable")) {
			...
			if (!ConfigurationManager
					.getBooleanProperty("webui.suggest.loggedinusers.only")) {
				// do nothing, the suggestLink stays as it is
			} else {
				// check whether there is a logged in user
				suggestEnable = (context.getCurrentUser() == null ? false
						: true);
			}
		}
		...
		new DSpace().getEventService().fireEvent(
				new UsageEvent(UsageEvent.Action.VIEW, request, context, item));
	...
	private void preProcessItemHome(Context context,
			HttpServletRequest request, HttpServletResponse response, Item item)
			throws ServletException, IOException, SQLException {
		try {
			ItemHomeProcessor[] chp = (ItemHomeProcessor[]) PluginManager
					.getPluginSequence(ItemHomeProcessor.class);
	...
	private void preProcessCommunityHome(Context context,
			HttpServletRequest request, HttpServletResponse response,
			Community community) throws ServletException, IOException,
			SQLException {
		try {
			CommunityHomeProcessor[] chp = (CommunityHomeProcessor[]) PluginManager
					.getPluginSequence(CommunityHomeProcessor.class);
			for (int i = 0; i < chp.length; i++) {
		...
		if (!handleButton(request, response, collection.getHandle())) {
			...
			if (e != null) {
				...
				if (AuthorizeManager.authorizeActionBoolean(context,
					...
					request.setAttribute("can_submit_button", Boolean.TRUE);

				} else {
					request.setAttribute("can_submit_button", Boolean.FALSE);
				}
			}
	...
	private void preProcessCollectionHome(Context context,
			HttpServletRequest request, HttpServletResponse response,
			Collection collection) throws ServletException, IOException,
			SQLException {
		try {
			CollectionHomeProcessor[] chp = (CollectionHomeProcessor[]) PluginManager
					.getPluginSequence(CollectionHomeProcessor.class);
			for (int i = 0; i < chp.length; i++) {
