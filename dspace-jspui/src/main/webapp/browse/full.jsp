...
<%
	...
	if (bix.getName() != null)
		sharedLink += "type="
				+ URLEncoder.encode(bix.getName(), "UTF-8");
	...
	sharedLink += "&amp;sort_by="
			...
			+ "&amp;rpp="
			+ URLEncoder.encode(
					Integer.toString(bi.getResultsPerPage()), "UTF-8")
			+ "&amp;etal="
			+ URLEncoder
					.encode(Integer.toString(bi.getEtAl()), "UTF-8")
			+ valueString;
...
<dspace:layout titlekey="browse.page-title" navbar="<%=layoutNavbar%>">
	...
	<div id="browse_navigation" class="well text-center">
		<form method="get" action="<%=formaction%>">
			...
			%>
			<span>Ir para</span> <select name="year">
				<option selected="selected" value="-1"><fmt:message
						key="browse.nav.year" /></option>
				<%
				...
				%>
			</select> <input type="submit" class="btn btn-default" value="Listar" /> <br />
			<label for="starts_with"><fmt:message
					key="browse.nav.type-year" /></label> <input type="text"
				name="starts_with" size="4" maxlength="4" />
			<%
				}
			...
			%>
			<span><fmt:message key="browse.nav.jump" /></span> <a
				class="label label-default" href="<%=sharedLink%>&amp;starts_with=0">0-9</a>
			<%
				for (char c = 'A'; c <= 'Z'; c++) {
			%>
			<a class="label label-default"
				href="<%=sharedLink%>&amp;starts_with=<%=c%>"><%=c%></a>
			<%
				}
			%><br /> <span><fmt:message key="browse.nav.enter" /></span> <input
				type="text" name="starts_with" />&nbsp;<input type="submit"
				class="btn btn-default" value="<fmt:message key="browse.nav.go"/>" />
			<%
				}
			%>
		</form>
...
--%>
			...
			<select name="sort_by">
				...
				%>
				<option value="<%=sortBy.getNumber()%>" <%=selected%>><fmt:message
						key="<%=mKey%>" /></option>
				<%
					}
							}
			...
			%>
			<label for="order"><fmt:message key="browse.full.order" /></label> <select
				name="order">
				<option value="ASC" <%=ascSelected%>>Crescente</option>
				<option value="DESC" <%=descSelected%>>Decrescente</option>
			</select> <label for="rpp"><fmt:message key="browse.full.rpp" /></label> <select
				name="rpp">
				<%
				...
				%>
			</select> <label for="etal"><fmt:message key="browse.full.etal" /></label> <select
				name="etal">
				<%
				...
				%>
			</select> <input type="submit" class="btn btn-default" name="submit_browse"
	...
	<div class="panel panel-primary">
		...
		<div class="panel-heading text-center">
			...
			<%
				if (bi.hasPrevPage()) {
			%>
			<a class="pull-left" href="<%=prev%>"><fmt:message
					key="browse.full.prev" /></a>&nbsp;
			<%
				}
			%>
			...
			<%
				if (bi.hasNextPage()) {
			%>
			&nbsp;<a class="pull-right" href="<%=next%>"><fmt:message
					key="browse.full.next" /></a>
			<%
				}
			%>
		</div>
		...
		<%
			if (bix.isMetadataIndex()) {
		%>
		<dspace:browselistsemcolunas browseInfo="<%=bi%>"
			emphcolumn="<%=bix.getMetadata()%>" />
		<%
			} else if (withdrawn || privateitems) {
		%>
		<dspace:browselistsemcolunas browseInfo="<%=bi%>"
			emphcolumn="<%=bix.getSortOption().getMetadata()%>" linkToEdit="true"
			disableCrossLinks="true" />
		<%
			} else {
		%>
